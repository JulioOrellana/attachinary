      ...
      def attachinary(options)
        ...
        if options[:single]
          ...
          define_method :"#{options[:scope]}_url=" do |url, upload_options = {}|
            ...
            send(:"#{options[:scope]}=", Cloudinary::Uploader.upload_large(url, upload_options))
          end
          ...
          define_method :"#{options[:singular]}_urls=" do |urls, upload_options = {}|
            upload_options.merge! resource_type: 'auto'
            send(:"#{options[:scope]}=", urls.map { |url| Cloudinary::Uploader.upload_large(url, upload_options) })
          end
        end
